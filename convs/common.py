...
class MeanShift3D(nn.Module):
    def __init__(self, pixel_range, n_channels, rgb_mean=None, rgb_std=None, sign=-1):
        super(MeanShift3D, self).__init__()
        ...
        self.shifter = nn.Conv3d(n_channels, n_channels, 1, 1, 0)
        std = torch.Tensor(rgb_std)
        self.shifter.weight.data = torch.eye(n_channels).view(n_channels, n_channels, 1, 1, 1) / std.view(n_channels, 1, 1, 1, 1)
        self.shifter.bias.data = sign * pixel_range * torch.Tensor(rgb_mean) / std
