...
def calc_ssim(tensors_dict):
    x = tensors_dict['x']
    x = x[:, x.size(1)//2:x.size(1)//2+1]
    out = tensors_dict['out']
    target = tensors_dict['target']
    target = target[:, target.size(1)//2:target.size(1)//2+1]
    ...
    for i in range(bs):
        ...
        print("targeti.shape : ", targeti.shape)
...
def save_tensors(opt, data_idx, tensors_dict):
    ...
    # x1 = x.squeeze()#.transpose((1, 2, 0))
    # targetd1 = target.squeeze()#.transpose((1, 2, 0))
    # x1[x1 > 1.0] = 1.0
    # x1[x1 < 0.0] = 0.
    # targetd1[targetd1 > 1.0] = 1.0
    # targetd1[targetd1 < 0.0] = 0.
    ...
    #input_fn_path = os.path.join(test_results_dir, 'input-' + filenames + fmt)
    #targetd_fn_path = os.path.join(test_results_dir, 'targetd-' + filenames + fmt)
    ...
    #imageio.imwrite(input_fn_path, x1)
    #imageio.imwrite(targetd_fn_path, targetd1)
    imageio.imwrite(compare_fn_path, compare_img)

def save_metrics(opt, data_idx, fi, filename, noise_loss, noise_psnr, out_loss, out_psnr, out_ssim):
    report_path = os.path.join(opt.test_results_dir, 'report2.csv')

    if not os.path.isfile(report_path):
        with open(report_path, 'w') as f:
            f.write("dataset,idx,filename,noise_loss,noise_psnr,loss,psnr,ssim\n")

    with open(report_path, 'a') as f:
        f.write("{},{},{},{:.8f},{:.8f},{:.8f},{:.8f},{:.8f}\n".format(
            opt.test_datasets[data_idx], fi, filename, noise_loss, noise_psnr, out_loss, out_psnr, out_ssim
        ))
...
def save_results(opt, data_idx, fi, filename, tensors_dict):
    ...
    np_x = tensor_x.detach().detach().to('cpu').numpy()
    # print('x.shape:', x.shape)
    # print('target.shape:', target.shape)
    np_targetd = np_target[:, n//2].squeeze()
    np_x1 = np_x_mean[:, 0].squeeze()
    ...
    #compare_img = np.concatenate((np_x1, np_x_mean, np_out, np_targetd), axis=1)
